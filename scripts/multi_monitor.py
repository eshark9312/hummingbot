...
class SimpleArbitrage(ScriptStrategyBase):
    ...
    def _balance_check_place_orders(self, base_amount: Decimal, exchange_buy: str, exchange_sell: str, pair:str):
        base_currency = pair.split('-')[0]
        # balance check
        exc_buy_quote_balance = float(self.connectors[exchange_buy].get_available_balance("USDT"))
        exc_sell_base_balance = float(self.connectors[exchange_sell].get_available_balance(base_currency))
